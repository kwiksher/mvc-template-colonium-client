--[[
Coronium module for Corona SDK v1.92.3
Copyright (c) 2014-2015 C. Byerley
Twitter: @coroniumio
...
function Coronium:updateUser( userDataTable, _callback )
  ...
  if userDataTable.password then
    userDataTable.password = mime.b64( userDataTable.password )
  end
...
function Coronium:loginUser( email, password, _callback )
  ...
  if email and password then
    queryTable = { email = email, password = password }
  else --probably facebook 
    queryTable = { fb_id = email }
  end
...
function Coronium:uploadFile( fileMetaTable, _callback, _progress_callback )
  ...
  local destDirectory = fileMetaTable.destDir or nil
  ...
  if contentType then
    ...
    local endpoint = self.FILE
    if destDirectory then
      endpoint = self.FILE .. "/" .. destDirectory
    end
...
function Coronium:onResponse( event )
  if event.phase == "ended" then
    ...
    --broadcast it with network status as well
    if e ~= nil then
      if _callback then
        _callback( e, event )
      else --use global event
        self.dispatcher:dispatchEvent( e, event )
      end
    else
...
function Coronium:getMimeType( filePath )
  ...
  elseif string.find( path, ".zip" ) ~= nil then
    mime = self.ZIP
  elseif string.find( path, ".doc" ) ~= nil then
    mime = self.DOC
  elseif string.find( path, ".docx" ) ~= nil then
    mime = self.DOC
  elseif string.find( path, ".xls" ) ~= nil then
    mime = self.XLS
  elseif string.find( path, ".xlsx" ) ~= nil then
    mime = self.XLS
